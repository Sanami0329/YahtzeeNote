        ...
        <div class="mx-auto my-4 py-6 px-10">
            <div class="flex flex-col items-center gap-6">
                ...
                <a href="{{ route('score.history') }}" class="block">
                    <flux:card class="max-w-2xl h-12 flex justify-between items-center gap-20 p-6 bg-white hover:bg-brand-yellow-400 hover:border-brand-yellow-600 rounded-lg shadow-md">
                        <div class="w-24 text-left text-base font-semibold hover:font-bold text-zinc-600">{{ __('最高スコア') }}</div>
                        <div class="w-16 text-right text-base text-zinc-600">{{ $highestScore }}</div>
                    </flux:card>
                </a>
                <a href="{{ route('subusers.show') }}" class="block">
                    <flux:card class="max-w-2xl h-12 flex justify-between items-center gap-20 p-6 bg-white hover:bg-brand-yellow-400 hover:border-brand-yellow-600 rounded-lg shadow-md">
                        <div class="w-24 text-left text-base font-semibold hover:font-bold text-zinc-600">{{ __('登録メンバー') }}</div>
                        <div class="w-16 text-right text-base text-zinc-600">{{ $registeredMembers }}</div>
                    </flux:card>
                </a>
            </div>
        </div>
